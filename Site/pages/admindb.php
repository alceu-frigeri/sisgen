  ...
  if($_SESSION['role']['isadmin'] & $sisgensetup) {
	switch($_POST['act']) {
		case 'Import Initial Data':
			if($_POST['importdata']) {
				...
				echo 'going over it...<br>';
					...
					$line = fgetcsv($file,512,',','"','"');
					while ($line = fgetcsv($file,512,',','"','"')) {
						DBinsertdept($line[0],$line[1],$line[2],$line[3],$line[4],$line[5],$line[6],$line[7],$line[8],$line[9],$line[10],$line[11],$line[12],$line[13]);
					}
					fclose($file);
					echo '<p>';
		...
		case 'Restore acc/unit tables':
			if($_POST['restoretables']) {
				...
				while ($line = fgetcsv($file,512,',','"','"')) {
					...
					$q = "SELECT * FROM `unit` WHERE `acronym` = '".$line[15]."';";
		...
		if($_POST['courseadjust']){
			...
			DBinsertgrade('CCA99','Etp.08','ENG0700b','CONTROLE AVANÇADO DE PROCESSOS','3','OB');
			...
			courseupdt('CCA99','ENG03048','OB','Etp.08');
