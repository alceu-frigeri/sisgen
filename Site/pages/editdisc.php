...
<?php 
	...
	if ($postedit & $can_discipline) {
		...
		formselectsqlX($anytmp,"SELECT * FROM unit ORDER BY unit.acronym;",'unitid',$_POST['unitid'],'id','acronym');
	}
  ...
  if ($postedit & $can_discipline) {
	  while ($sqlrow=$result->fetch_assoc()) {
		...
		if ($_POST['discid'] == $sqlrow['id']) {
			...
			if ($sqlrow['comment']) {
				echo '&nbsp;&nbsp;&nbsp;'.spanformat('font-size:smaller;color:'.$commentcolor.';','('.$sqlrow['comment'].')') ;
			}
			echo '<br>';
		}
		echo "</form>";
