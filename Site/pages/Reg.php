...
function strform($phase,$validate,$target='') {
    ...
    if ($loginacc) {
	$str .= "<input type='hidden' name='hash' value='$loginacc[hashval]' />\n";
    } 
    if ($loginadmin) {
	$str .= "<input type='hidden' name='hash2' value='$loginadmin[hashval]' />\n";
...
function mail_logincheck($email,$passwd) {
	...
	//	echo "hash ORG: $sqlrow[hashval]<br>\n";
	if (($passwd == $sqlrow['password'] && $sqlrow['activ'])) {
	    $newhash=md5("$email - $microstamp");
	    $result->close();
	    if (!($mysqli->query("UPDATE  `account` SET `hashval` = '$newhash' WHERE `email` = '$email';"))) {
		$err = "Query failed: (" . $mysqli->errno . ") " . $mysqli->error;
		eventlog('DBERROR','',$err) ;
	    }
	    //            return array($newhash,$sqlrow['can_edit'],$sqlrow['type_id']);
	    $sqlrow['hashval']=$newhash;
	    //	    echo "hash NEW: $sqlrow[hashval]<br>\n";
...
function hash_logincheck($hash) {
    ...
    if (!($result = $mysqli->query("SELECT * FROM `account` WHERE `hashval` = '$hash';"))) {
	...
	if ($sqlrow['activ']) {
	    ...
	    //	    $result->close();
	    //	    if (!($mysqli->query("UPDATE  `account` SET `hashval` = '$newhash' WHERE `hashval` = '$hash';"))) {
	    //		echo "Query failed: (" . $mysqli->errno . ") " . $mysqli->error;
	    //	    }
	    //	    $sqlrow['hashval']=$newhash;
	    $result->close();
	    if (!($mysqli->query("UPDATE  `account` SET `last_action` = NOW() WHERE `hashval` = '$hash';"))) {
	    	echo "Query failed: (" . $mysqli->errno . ") " . $mysqli->error;
	    }
	    return $sqlrow;
	}
